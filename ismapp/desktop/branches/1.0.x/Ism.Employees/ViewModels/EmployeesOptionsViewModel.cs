...
namespace Ism.Employees.ViewModels
{
    public class EmployeesOptionsViewModel : ViewModelBase
    {
        ...
        public EmployeesOptionsViewModel(ISettingsService settings, ISecurityService security, IExceptionService exceptionService, INavigationService navigationService)
        {
            try
            {
                ...
                {
                    Parent = navigationService.GetParents().Where(p => p.Title == "Zaposleni").FirstOrDefault(),
                    Title = "Seznam",
            ...
            try
            {
                ...
                using (var repositroy = _serviceLocator.GetInstance<IRestRepository<Employee, Employee>>())
                {
                    ...
                    repositroy.PostRequestAsync(url.ToString(), employee,
                        _security.GetCurrentUser().AccessToken,
                        (e) =>
                        {
                            OnEmployeesList();
                        });
