    ...
    class UserEditViewModel : ViewModelBase, IInteractionRequestAware
    {
        ...
        private void AddUser(User user)
        {
            try
            {
                ...
                {
                    rep.PostRequestAsync(new Uri(_settingsService.GetAuthServer(), "/users/add").ToString(), user, _securityService.GetCurrentUser().AccessToken, (u) =>
                    {
                        FinishInteraction();
                    });
                }
        ...
        private void UpdateUser(User user)
        {
            try
            {
                ...
                using (var rep = _serviceLocator.GetInstance<IRestRepository<User, User>>())
                {
                    rep.PostRequestAsync(new Uri(_settingsService.GetAuthServer(), $"/users/update").ToString(), user, _securityService.GetCurrentUser().AccessToken, (u) =>
                    {
                        FinishInteraction();
                    });
                }
        ...
        private void RefreshScopes()
        {
            try
            {
                ...
                {
                    rep.GetRequestAsync(new Uri(_settingsService.GetAuthServer(), $"/scope/list").ToString(),_securityService.GetCurrentUser().AccessToken, (list) =>
                    {
                ...
                scope.PropertyDeletegate = (model) =>
                {
                    ...
                    try
                    {
                        using (var rep = _serviceLocator.GetInstance<IRestRepository<User, Scope>>())
                        {
                            ...
                            rep.PostRequestAsync(url, mscope, _securityService.GetCurrentUser().AccessToken, (u) =>
                            {
                                int a = 0;
                            });
                        }
