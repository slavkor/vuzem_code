        ...
        private void RefreshEwrs(Range range)
        {
            try
            {
                ...
                using (var repository = _serviceLocator.GetInstance<IRestRepository<List<Ewr>, Range>>())
                {
                    ...
                    repository.PostRequestAsync(new Uri(_settingsService.GetApiServer(), $"project/{Project.UuId}/ewr/list").ToString(), range, _securityService.GetCurrentUser().AccessToken,
                        (e) =>
                        {
