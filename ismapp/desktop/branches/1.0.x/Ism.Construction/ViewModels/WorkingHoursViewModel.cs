...
namespace Ism.Construction.ViewModels
{
    class WorkingHoursViewModel : ViewModelBase
    {
        ...
        public WorkingHoursViewModel(IExceptionService exceptionService, ISettingsService settingsService, ISecurityService securityService, IDepartureService departures)
        {
            ...
            HoursToAll = new DelegateCommand<object>(OnHoursToAll);
        }



        private void OnHoursToAll(object obj)
        {
            try
            {
                int hours = 0;
                int.TryParse(obj.ToString(), out hours);
                if (hours == 0) return;

                var last = EmployeeShifts.Last();
                foreach (var employee in EmployeeShifts)
                {
                    var shift = new Shift() { UuId = Guid.NewGuid().ToString(), ShiftType = 0, WorkDay = new Day(SelectedDate), Hours = hours };
   
                    using (var rep = _serviceLocator.GetInstance<IRestRepository<object, Shift>>())
                    {
                        rep.PostRequestAsync(new Uri(_settingsService.GetApiServer(), $"project/{Project.UuId}/employee/{employee.Employee.UuId}/shiftadd").ToString(), shift, _securityService?.GetCurrentUser()?.AccessToken, (l) => {
        ...
        public DelegateCommand<object> HoursToAll { get; }
        ...
        private void RefreshEmployees(DateTime day)
        {
            try
            {
                ...
                {
                    rep.PostRequestAsync(new Uri(_settingsService.GetApiServer(), $"project/{Project.UuId}/workdayshifts").ToString(), new Day(day), _securityService.GetCurrentUser().AccessToken, (list) =>
                    {
                        try
                        {
        ...
        private void ShiftEventHandler(BaseModel model)
        {
            try
            {
                ...
                using (var rep = _serviceLocator.GetInstance<IRestRepository<object, Shift>>())
                {
                    rep.PostRequestAsync(new Uri(_settingsService.GetApiServer(), $"project/{Project.UuId}/employee/{SelectedEmployeeShift.Employee.UuId}/shiftadd").ToString(), shift, _securityService?.GetCurrentUser()?.AccessToken, (l) =>
                    {
        ...
        private void RefreshShifts()
        {
            ...
            {
                rep.PostRequestAsync(new Uri(_settingsService.GetApiServer(), $"project/{Project.UuId}/allshifts").ToString(), null, _securityService.GetCurrentUser().AccessToken, (list) =>
                {
                    try
                    {
                        if (list == null)
