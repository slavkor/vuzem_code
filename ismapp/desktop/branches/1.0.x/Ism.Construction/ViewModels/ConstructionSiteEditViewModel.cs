    ...
    public class ConstructionSiteEditViewModel : ViewModelBase
    {
        ...
        private void DownloadConstructionSiteImage(ConstructionSite obj)
        {
            try
            {
                ...
                using (var rep = _serviceLocator.GetInstance<IRestRepository<ConstructionSite, string>>())
                {
                    rep.GetRequestAsync(new Uri(_settings.GetApiServer(), $"ConstructionSites/{obj.UuId}/document/{commons.GetDocumentTypes().FirstOrDefault(t => t.Name == "SLIKA")?.Name}").ToString(), _securityService.GetCurrentUser().AccessToken, ConstructionSite =>
                        {
