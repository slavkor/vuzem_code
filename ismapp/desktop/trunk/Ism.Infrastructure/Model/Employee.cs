...
namespace Ism.Infrastructure.Model
{
    ...
    public class Employee : BaseModel, ISelectionAware
    {
        ...
        [JsonIgnore]
        public Company CurrentEmployer
        {
            get
            {
                if (null == WorkPeriods) return null;
                var c = WorkPeriods.OrderByDescending(w => w.Start).FirstOrDefault()?.Company;
                return c;
            }
        }
        ...
        public WorkPeriod CurrentWorkPeriod
        {
            ...
            {
                if (null == WorkPeriods) return null;
                var c = WorkPeriods.OrderByDescending(w => w.Start).FirstOrDefault();
                return c;
            }
        }

        [JsonIgnore]
        public bool NotEmployed
        {
            get
            {
                if (null == WorkPeriods) return false;
                var p = WorkPeriods.OrderByDescending(w => w.Start).FirstOrDefault();
