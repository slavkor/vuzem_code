...
namespace Ism.Repository
{
    public class RestRepositroy<T, TP> : IRestRepository<T, TP>
    {
        ...
        {
            return PostRequest(uri, payLoad, _securityService?.GetCurrentToken());
        }
        ...
        {
            return GetRequest(url, _securityService?.GetCurrentToken());
        }
        ...
        {
            GetRequestAsync(url, _securityService?.GetCurrentToken(), callback, "Počakaj...", true);
        }
        ...
        {
            PostRequestAsync(uri, payLoad, _securityService?.GetCurrentToken(), callback, "Počakaj...");
        }
        ...
        public async void PostFileAsync(string url, string fileName, Token token, IDictionary<string, string> querystring, Action<T> callback, string waitNotification, bool busyNotification)
        {
            BusyEventArgs busyEventArgs = null;
            try
            {
        ...
        public void GetFileAsync(string url, Token token, IDictionary<string, string> querystring, Action<T> callback, string waitNotification, bool busyNotification)
        {
            BusyEventArgs busyEventArgs = null;
            try
            {
        ...
        public void GetReportAsync(Report report, Token token, Action<T> callback, string waitNotification, bool busyNotification, Action<Exception> errcallback)
        {
            BusyEventArgs busyEventArgs = null;
            try
            {
        ...
        public Stream GetFile(string url, Token token, IDictionary<string, string> querystring)
        {
            Stream response = default(Stream);
            try
            {
                string postdata = "";
                if (null != querystring)
                ...
                var webRequest = (HttpWebRequest)WebRequest.Create(url +postdata);
                webRequest.ContentType = "application/json;charset=utf-8";
                webRequest.Method = "GET";
                if (null != token)
                {
