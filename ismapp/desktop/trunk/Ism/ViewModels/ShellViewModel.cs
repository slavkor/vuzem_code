...
namespace Ism.ViewModels
{
    ...
    public class ShellViewModel : WindowAware
    {
        ...
        {
            bool busy = (bool)e.Argument;
            EnableDisableBusyIndicatorAsync(busy);
        }
        ...
        private void OnBusyEvent(BusyEventArgs args)
        {
            ...
            //worker.RunWorkerAsync(args.Busy);
            //isBusy = args.Busy;
        }
        ...
        private void OnLoginEvent(Token token)
        {
            try
            {
                // set current user
                ...
                security.SetCurrentToken(token);
                ...
                if (null != common)
                {
                    ...
                    common.FetchShared<List<Company>>(@"/company/list", list =>
                    {
                        try
                        {
                            ...
                            {
                                rep.GetRequestAsync(new Uri(settings.GetApiServer(), $"/employees/{token.GetClaim("sub")}").ToString(), security.GetCurrentToken(), (employee) =>
                                {
                                    try
                                    {
                                        if (null == employee)
                                        ...
                                        {
                                            repo.GetRequestAsync(new Uri(settings.GetApiServer(), $"csite/byemployee/{security.GetCurrentEmployee().UuId}").ToString(), security.GetCurrentToken(), (site) =>
                                            {
                                                try
                                                {
