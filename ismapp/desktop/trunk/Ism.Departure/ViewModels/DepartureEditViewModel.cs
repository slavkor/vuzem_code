    ...
    class DepartureEditViewModel : ViewModelBase, IInteractionRequestAware
    {
        ...
        public DepartureEditViewModel(ISettingsService settings, ISecurityService securityService, IExceptionService exceptionService)
        {
            ...
            try
            {
                ...
                DriverCommand = new DelegateCommand<EmployeeListItem>(OnDriverCommand);
                PassengerCommand = new DelegateCommand<EmployeeListItem>(OnPassengerCommand);
        ...
        public DelegateCommand<EmployeeListItem> DriverCommand { get; }
        public DelegateCommand<EmployeeListItem> PassengerCommand { get; }
        ...
        private void OnSelectEmployees2(List<EmployeeDepature> list)
        {
            try
            {
                ...
                //    }
                //}
                using (var repository = _serviceLocator.GetInstance<IRestRepository<Employee, List<Employee>>>())
                {
                    repository.PostRequestAsync(new Uri(_settingsService.GetApiServer(), $"departures/{Departure.UuId}/employee/addmany").ToString(), emps, _securityService.GetCurrentToken(), (e) =>
                    {
                        try
                        {
                            RefreshDepartureEmployees();
        ...
        private void RefreshDepartureEmployees()
        {
            try
            {
                ...
                {
                    repository.GetRequestAsync(new Uri(_settingsService.GetApiServer(), $"departures/{Departure.UuId}/employees").ToString(), _securityService.GetCurrentToken(), (list) =>
                    {
                        try
                        {
                            Employees = new ObservableCollection<EmployeeListItem>(list.Select(e => new EmployeeListItem() { Employee = e, RemoveEmployee = EmployeeRemoveCommand, StateChangeCommand = EmployeeStateChangeCommand, DriverCommand = DriverCommand, PassengerCommand = PassengerCommand }));
                            Employees.Add(new EmployeeListItem() { IsAddItem = true, AddEmployee = EmployeeSelectCommand });
                        }
                        catch (Exception exc)
                        {
        ...
        private void RefreshDepartureCars()
        {
            try
            {
                using (var repository = _serviceLocator.GetInstance<IRestRepository<IList<Car>, object>>())
                {
                    ...
                    repository.GetRequestAsync(new Uri(_settingsService.GetApiServer(), $"departures/{Departure.UuId}/cars").ToString(), _securityService.GetCurrentToken(), (list) =>
                    {
                        try
                        {
            ...
            try
            {
                ...
                using (var repository = _serviceLocator.GetInstance<IRestRepository<Car, Car>>())
                {
                    foreach (var item in cars)
                    {
                        ...
                        repository.PostRequestAsync(new Uri(_settingsService.GetApiServer(), $"departures/{Departure.UuId}/car/add").ToString(), item as Car, _securityService.GetCurrentToken(), (c) => {
        ...
        private void OnEmployeeRemoveCommand(EmployeeListItem  employee)
        {
            try
            {
                ...
                using (var repository = _serviceLocator.GetInstance<IRestRepository<Employee, Employee>>())
                {
                        repository.PostRequestAsync(new Uri(_settingsService.GetApiServer(), $"departures/{Departure.UuId}/employee/remove").ToString(), employee.Employee as Employee, _securityService.GetCurrentToken(), (e) => {
                            try
                            {
                                RefreshDepartureEmployees();
                            }
        ...
        private void OnCarRemoveCommand(CarListItem obj)
        {
            ...
            using (var repository = _serviceLocator.GetInstance<IRestRepository<Car, Car>>())
            {
                repository.PostRequestAsync(new Uri(_settingsService.GetApiServer(), $"departures/{Departure.UuId}/car/remove").ToString(), obj.Car as Car, _securityService.GetCurrentToken(), (e) => {
                    try
                    {
                        RefreshDepartureCars();
                    }
        ...
        private void OnDriverCommand(EmployeeListItem obj)
        {
            try
            {
                using (var repository = _serviceLocator.GetInstance<IRestRepository<object,object>>())
                {
                    repository.PostRequestAsync(new Uri(_settingsService.GetApiServer(), $"departures/{Departure.UuId}/employee/{obj.Employee.UuId}/driver").ToString(), null, _securityService.GetCurrentToken(), (e) =>
                    {
                        try
                        {
                            RefreshDepartureEmployees();
        ...
        private void OnPassengerCommand(EmployeeListItem obj)
        {
            try
            {
                using (var repository = _serviceLocator.GetInstance<IRestRepository<object, object>>())
                {
                    repository.PostRequestAsync(new Uri(_settingsService.GetApiServer(), $"departures/{Departure.UuId}/employee/{obj.Employee.UuId}/passenger").ToString(), null, _securityService.GetCurrentToken(), (e) =>
                    {
                        try
                        {
                            RefreshDepartureEmployees();
