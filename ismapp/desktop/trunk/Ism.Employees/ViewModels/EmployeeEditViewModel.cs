    ...
    public class EmployeeEditViewModel : ViewModelBase
    {
        ...
        private void OnTabSelectionChangedCommand(object obj)
        {
            try
            {
                ...
                int.TryParse(tabitem.Tag.ToString(), out tabIndex);

                var par = new NavigationParameters();
                par.Add("navigation", new EditInteraction<Employee>()
                {
                    Content = Employee
                ...
                {
                    case 1:
                        _regionManager.RequestNavigate(RegionNames.VacSickLeaveRegion, "VacSickLeaveView", (args) => { }, par);
                        break;
                    case 2:
                        _regionManager.RequestNavigate(RegionNames.EmployeeHistoryRegion, "EmployeeHistoryView", (args) => { }, par);
                        break;
                    default:
                        break;
                }
        ...
        private void SaveRelatedData()
        {
            try
            {
                ...
                if (null != Employee.Loaner)
                {
                    ...
                    {
                        Uri uri = new Uri(_settings.GetApiServer(), $"employees/{Employee.UuId}/addloaner");
                        rep.PostRequestAsync(
                            uri.ToString(),
                            Employee.Loaner, (e) => { });
                    }
        ...
        private void RefreshWorkPeriod()
        {
            try
            {
                ...
                using (var rep = _serviceLocator.GetInstance<IRestRepository<CurrentWorkPeriod, object>>())
                {
                    ...
                    rep.GetRequestAsync(url.ToString(), _securityService.GetCurrentToken(),
                        currentwork =>
                        {
            ...
            try
            {
                ...
                using (var rep = _serviceLocator.GetInstance<IRestRepository<EmployeeList, object>>())
                {
                    if (data.CompanyHire.ShortName == data.CompanyFire?.ShortName)
                    {
                        ...
                        var periodh = rep.GetRequest(url.ToString(), _securityService.GetCurrentToken());
                    ...
                    else
                    {
                        if (null != data.CompanyFire)
                        {
                            ...
                            url = new Uri(_settings.GetApiServer(data.CompanyFire.UuId),
                                $"employees/{data.Employee.UuId}/workperiods/current");
                            var periodf = rep.GetRequest(url.ToString(), _securityService.GetCurrentToken());
        ...
        private void DownloadEmployeeImage(Employee obj)
        {
            try
            {
                ...
                using (var rep = _serviceLocator.GetInstance<IRestRepository<Employee, string>>())
                {
                    rep.GetRequestAsync(new Uri(_settings.GetApiServer(), $"employees/{obj.UuId}/document/{commons.GetDocumentTypes().FirstOrDefault(t=> t.Name == "SLIKA")?.Name}").ToString(), _securityService.GetCurrentToken(), employee =>
                        {
            ...
            try
            {
                ...
                switch (_editMode)
                {
                    ...
                    default:
                        if(null == obj)
                        {
                            _eventAggregator.GetEvent<ConfirmSaveEvent<BaseModel>>().Publish(new ConfirmSaveEventArgs<BaseModel>() { CallBackAction = (confirmed, payload) => {
                                if (!confirmed) return;
                                Uri uri = new Uri(_settings.GetApiServer(), $"employees/{Employee.UuId}/deleteloaner");
                            ...
                            }, Title = "ALO", Content = "Želiš nadaljevati? Zaposelni bo voden kot neizposojen delavec!", FinishUp = true, PayLoad = obj });
                        ...
                        else
                        {
                            ...
                            {
                                CallBackAction = (confirmed, payload) => {
                                    if (!confirmed) return;
                                    Uri uri = new Uri(_settings.GetApiServer(), $"employees/{Employee.UuId}/addloaner");
                                    using (var rep = _serviceLocator.GetInstance<IRestRepository<BaseModel, BusinessPartner>>())
                                    {
                                ...
                                Title = "ALO",
                                Content = "Želiš nadaljevati? Zaposelni bo voden kot izposojen delavec!",
                                FinishUp = true,
                                PayLoad = obj
                            });
